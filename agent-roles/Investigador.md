...
- Verifies every citation against the source's **full text** per `reference-writting/source-fidelity.md` before handing it to the Redactor: the evidence brief carries, for each claim, the literal passage with its locator and whether it is the source's own finding, and each checked use is recorded in `thesis/CITAS-VERIFICADAS.md`. A term, figure or qualifier that the passage doesn't contain does not go into the brief as the source's claim.
- Obtains sources through `fuentes/README.md`: open access first, then the university's licensed portals (ACM Digital Library as the main one, Applied Science & Technology Source, Knovel), then preprints. Every PDF read is stored as `fuentes/pdf/<bibtex-key>.pdf` and listed in `fuentes/INDEX.md`. When a portal blocks the download, the Investigador adds the key to the index's pending table and asks a person for the file instead of working from the abstract. Claims about the state of a field (gaps, lack of consensus) come from the newest review available (`reference-writting/recency.md`).
