...
> `alcance-moscow.md` (the MoSCoW scope ladder) was **deleted on 2026-09-20**; its capabilities were folded into `requirements.md`, which is now the only requirements document. Its former `A-NN` row ids survive only in the *Origen* column of that file.
