...
- **Dónde se usa:**
  ...
  - `thesis/chapters/07-metodologia.tex` — `\item[Saturación de VRAM y congelamiento de nodos GPU]` (rango de 90 % a 95 % y fuente de poder) · marcador literal: `% [verify: ADR-030: rango 90-95 % y fuente de poder salen del acta 2026-09-04; requirements.md R1-11 no fija el umbral de saturación]`
...
- **Avance (2026-09-13):** en la revisión del acta del 2026-08-26 los autores promovieron el contenido de los *sprints* 1 y 2 junto con la exigencia del curso (`project-context/README.md`, «Course requirement for PDG I»), el calendario de PDG I y PDG II (`project-context/README.md`, «Academic calendar», que corrige el corte a los primeros días de diciembre) y la restricción de uso compartido con las clases (`requirements.md` R3-08). Esos marcadores ya se borraron del cap. 07. El nuevo calendario abrió ADR-033.
...
- **Dónde se usa:**
  ...
  - `project-context/requirements.md` — R4-01 a R4-03
...
- **Al resolver:** ajustar R4-01 a R4-03 y §E.1 al origen decidido, cambiar el verbo de la fila *Sprint* 15 si aplica, revisar las dos menciones del cap. 08 y borrar el marcador.
...
- **Dónde se usa:**
  - `project-context/requirements.md` — §2, las cuatro tablas por objetivo
  ...
  - `project-context/requirements.md` — los requerimientos de las filas que queden fuera
...
- **Avance (2026-09-21):** el tutor devolvió `requirements.md` con comentarios y los autores aplicaron los cambios. Los que tocan el alcance son: el clúster se despliega primero en la sala 104M y debe ser extensible a otras salas (§1.2 y R3-01); la inferencia distribuida entre nodos deja de ser una exclusión absoluta y la plataforma queda preparada para habilitarla vía Ray cuando la red lo permita (§1.3 y R3-17); y las alertas se confirman como configuración del stack (R1-30). Sigue sin haber un corte de alcance acordado con el tutor.
- **Al resolver:** reescribir los objetivos afectados (y su espejo en `documentation.md`), marcar fuera de alcance los requerimientos de las filas excluidas, cerrar ADR-029 y ADR-031 con la misma decisión y abrir el trabajo de ADR-033.
...
- **A quién corresponde:** autores / tutor
- **Dónde se usa:**
  - `project-context/requirements.md` — §2.2.3, R2-20, columna de requerimiento · marcador literal: `*[verify: ADR-034]*`
  ...
  - `thesis/chapters/06-estado-del-arte.tex` — §«Síntesis comparativa», párrafo final · texto literal: `con prioridad docente`
- **Contexto:** los autores fijaron el 2026-09-20 que el modelo de roles es el que dejó el tutor el 2026-09-09, «administrador y usuario, de momento». Con dos roles no existe un nivel profesor/estudiante, de modo que R2-20 solo puede implementarse como un atributo de prioridad configurable. Pero el objetivo específico 2 del anteproyecto sí promete «prioridad de los profesores sobre la asignación de la capacidad de la sala», y los capítulos 07 y 08 lo repiten. O el objetivo se reescribe en términos de prioridad por rol, o se agrega el nivel profesor/estudiante al modelo de roles. Es la tensión que ADR-032 no resolvió.
...
- **Al resolver:** aplicar la opción elegida en R2-20 de `requirements.md`, borrar el marcador, y reescribir `obj:gobernanza` y sus ecos en los capítulos 07 y 08 si aplica.
