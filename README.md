...
- **System Requirements** ([`requirements.md`](./project-context/requirements.md)): the development specification the team builds against (model provisioning, reservation-based quotas with role priority, 10-20% overbooking, SAAMFI RBAC, hardware and workload telemetry, measurement engine) plus non-functional constraints. **It is a two-tier document:** every requirement in section 2 is built and may be written into `thesis/`; the requirements in section 3 (`Sin compromiso`: training workloads, external consumption, course-level reservation priority) are development agreements that do **not** widen the PDG scope and must never appear in the thesis.
- **Technology Stack:** Multi-tiered architecture encompassing Kubernetes, KubeRay, NVIDIA GPU Operator, vLLM, Ollama, llama.cpp, the PLG monitoring stack (Prometheus/Loki/Grafana), and DCGM telemetry. LiteLLM Proxy is **not** adopted as the AI gateway (ADR-024).
