...
| Marco teórico | `chapters/05-marco-teorico.tex` | under review | **8 / 8 pág.** (tope propio, no el del formato) | **Reescrito 2026-09-13.** Ocho secciones de bases teóricas continuas, de lo general a lo específico. Cierra con "Marco conceptual y categorías de análisis" y su tabla concepto → definición operativa → objetivo, en vez de repetir un `\cref{obj:...}` al final de cada párrafo. Pesos relativos fijados por los autores: CRDs e identidad breves, contenedores resumido. |
| Estado del arte | `chapters/06-estado-del-arte.tex` | not started | máx. 4 pág. | **Único capítulo del cuerpo sin redactar.** Material pendiente: plataformas MLOps en nube, clústeres GPU de campus, LiteLLM como antecedente descartado, **TensorFlow Serving y Kubeflow** (señalados por los autores en `busquedas-realizadas.md`) y la **comparación vLLM contra llama.cpp** que salió del Marco teórico el 2026-09-13 por límite de extensión. |
| Metodología | `chapters/07-metodologia.tex` | under review | 7 pág. (sin límite) | Sin DSR. **Rehecha 2026-09-13:** sprints de **tres semanas** desde el 21-sep-2026, cinco fases en orden clúster → telemetría → cuotas → plataforma → evaluación, sin fase de mockups, entrega del anteproyecto a inicios de diciembre de 2026. |
| Contribución y resultados | `chapters/08-contribucion-resultados.tex` | under review | **3 / 4 pág.** | Cumple. Trazabilidad objetivo → resultado → entregable realineada el 2026-09-13 con los objetivos 1 y 3 reformulados. |
| Anexos | `chapters/99-anexos.tex` | not started | — | Bloqueado por ADR-010: falta decidir si los árboles van en TikZ o como imagen. |
| Bibliografía | `references.bib` | drafted | — | 43 entradas (se añadieron `burns-borgomegak8s-2016` y `xu-k8soperatorbugs-2024` para CRDs y patrón operador). **Ninguna fuente anterior a 2015**: se retiraron `hevner-designscienceis-2004`, `peffers-dsrm-2007` y `brooke-sus-1996`; `yoo-slurm-2003` y `vavilapalli-yarn-2013` siguen en el archivo pero ya no se citan. |
...
- Cada capítulo dentro de su extensión máxima institucional, salvo el Marco teórico por decisión expresa de los autores: cap. 01 = 3/3, cap. 02 = 1/1, cap. 04 = 1/1, cap. 08 = 3/4. El cap. 05 va en 8 páginas, bajo su tope propio.
